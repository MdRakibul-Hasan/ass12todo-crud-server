...
// Update products 
// ===================================
app.patch('/techProduct/:id', async(req, res) => {
  const id = req.params.id;
  const filter = { _id: new ObjectId(id) }
  // const options = { upsert: true};
  ...
  const updatedDoc = {
    ...
    const result = await allProductCollection.updateOne(filter, updatedDoc)
    res.send(result);
})
...
app.put('/techProduct/:id', async(req, res) => {
  ...
  const techProduct = {
      $set: {
        report: updatedproduct.report, 

      }
  }
    const result = await allProductCollection.updateOne(filter, techProduct, options)
    res.send(result);
})
